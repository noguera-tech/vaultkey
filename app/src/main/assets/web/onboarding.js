...
(function (root, factory) {
  ...
  function visibilityButton(targetId, label) {
    var eye = '<svg class="vk-onb-visibility__icon vk-onb-visibility__icon--open" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2.1 12s3.6-6 9.9-6 9.9 6 9.9 6-3.6 6-9.9 6-9.9-6-9.9-6"/><circle cx="12" cy="12" r="2.5"/></svg>' +
      '<svg class="vk-onb-visibility__icon vk-onb-visibility__icon--off" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m3 3 18 18"/><path d="M10.6 10.6a2 2 0 0 0 2.8 2.8"/><path d="M9.9 4.2A10.8 10.8 0 0 1 12 4c6.3 0 9.9 8 9.9 8a17.8 17.8 0 0 1-2.1 3.1"/><path d="M6.2 6.2C3.6 8 2.1 12 2.1 12s3.6 8 9.9 8a9.8 9.8 0 0 0 4.1-.9"/></svg>';
    return '<button class="vk-onb-visibility" type="button" data-ob="toggle-visibility:' +
      targetId + '" aria-label="Mostrar ' + label + '" aria-pressed="false">' + eye + '</button>';
  }
          ...
          '<form class="vk-onb-master__form" onsubmit="return false">' +
            '<label class="vk-onb-master__field">' +
              '<span class="vk-onb-master__control">' +
                ...
                visibilityButton('ob-master', 'contraseña maestra') +
              '</span>' +
            ...
            '<label class="vk-onb-master__field">' +
              '<span class="vk-onb-master__control">' +
                ...
                visibilityButton('ob-master2', 'confirmación de contraseña') +
              '</span>' +
        ...
        '<div class="vk-onb-pin__content">' +
          ...
          '<form class="vk-onb-pin__form" onsubmit="return false" autocomplete="off">' +
            ...
            '<label class="vk-onb-pin__field">' +
              ...
              visibilityButton('ob-pin2', 'confirmación del PIN') +
    ...
    'onboarding-kit-verify': function () {
      return '<section class="vk-onb-kit-verify">' +
        ...
        '<div class="vk-onb-kit-verify__block">' +
          ...
          '<p class="vk-onb-kit-verify__help">Descarga o imprime una copia y guárdala en un lugar seguro.</p>' +
  ...
  function handleAction(action, ctx) {
    ...
    if (action.indexOf('toggle-visibility:') === 0) {
      ...
      var input = root.document.getElementById(targetId);
      var toggle = root.document.querySelector('[data-ob="' + action + '"]');
      if (!input) { return; }

      var reveal;
      if (targetId === 'ob-pin' || targetId === 'ob-pin2') {
        reveal = input.style.webkitTextSecurity !== 'none';
        input.style.webkitTextSecurity = reveal ? 'none' : 'disc';
        input.style.textSecurity = reveal ? 'none' : 'disc';
      } else {
      ...
      if (toggle) {
        ...
        toggle.setAttribute('aria-label', (reveal ? 'Ocultar ' : 'Mostrar ') +
          (targetId.indexOf('pin') !== -1 ? 'PIN' : 'contraseña'));
      }
      input.focus();
      return;
    }
    ...
    if (action === 'submit-kit-verify') {
      ...
      if (typed !== expected) {
        haptic('error');
        setHint('ob-kit4-hint', '', '');
        if (typeof root.toast === 'function') {
          root.toast('No coincide. Comprueba los 4 últimos caracteres de tu código.', 'err');
        }
        return;
      }
